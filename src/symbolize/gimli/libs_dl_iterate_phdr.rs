...
unsafe extern "C" fn callback(
    ...
    #[cfg(target_os = "android")]
    let apk_offset = {
        // only check for APK-embedded file if we have data from /proc/self/maps
        maps.as_ref().and_then(|maps| {
            // check if file is embedded within an APK by searching for `.apk!/`
            memchr::memmem::find(name.as_bytes(), b".apk!/").and_then(|match_index| {
                // find MapsEntry matching library's base address
                let offset = maps.iter()
                    .find(|m| m.ip_matches(dlpi_addr as usize))
                    .map(|m| m.offset());

                // truncate name to APK path
                let mut name_bytes = core::mem::take(&mut name).into_vec();
                name_bytes.truncate(match_index + 4);
    ...
    libs.push(Library {
        name,
        #[cfg(target_os = "android")]
        apk_offset: apk_offset.unwrap_or(0),
        segments: headers
            .iter()
